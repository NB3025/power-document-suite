...
│       └── 페이지 병합 → 저장
└── 좌표 확인 필요?
    ├── pdfplumber로 텍스트 위치 분석
    └── 좌표 변환 (pdfplumber → reportlab)
```
...
폼 필드가 없는 PDF에는 텍스트 오버레이를 사용합니다.
...
1. PDF → 이미지 변환 (위치 분석용)
2. 이미지에서 입력 영역 좌표 확인
3. 바운딩 박스 정의 (레이블과 입력 영역 분리)
4. reportlab으로 텍스트 오버레이 생성
5. 원본 PDF와 오버레이 병합
```

### Step 1: PDF를 이미지로 변환 (위치 분석용)
    ...
    "form_fields": [
        {
            ...
            "field_label": "Name:",
            "label_bounding_box": [72, 100, 130, 115],  # 레이블 영역
            "entry_bounding_box": [135, 100, 400, 115], # 입력 영역 (겹치지 않게!)
            "entry_text": {
                "text": "홍길동",
                "font_size": 12,
                "font_color": "000000"
        ...
        },
        {
            ...
            "description": "동의 체크박스",
            "field_label": "I agree",
            "label_bounding_box": [100, 500, 160, 515],
            "entry_bounding_box": [72, 500, 90, 515],  # 체크박스 영역만!
            "entry_text": {
                "text": "X",  # 체크 표시
                "font_size": 14
            }
...
### 텍스트가 잘못된 위치에 표시됨
- **원인**: 좌표 체계 혼동 (pdfplumber vs reportlab)
- **해결**: `pdfplumber_to_reportlab()` 변환 사용

### 한글이 깨짐
- **원인**: 한글 폰트 미등록
- **해결**: TTFont로 한글 폰트 등록 후 사용
...
- **해결**: `validate_form_fields()`로 겹침 검사 후 수정
